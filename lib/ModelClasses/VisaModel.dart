class VisaModel {
  String _massage;
  bool _status;
  List<VisaDetail> _data;

  VisaModel({String massage, bool status, List<VisaDetail> data}) {
  ...
  List<VisaDetail> get data => _data;
  set data(List<VisaDetail> data) => _data = data;

  VisaModel.fromJson(Map<String, dynamic> json) {
    _massage = json['massage'];
    _status = json['status'];
    if (json['data'] != null) {
      _data = new List<VisaDetail>();
      json['data'].forEach((v) {
        _data.add(new VisaDetail.fromJson(v));
      });
    }
  }
